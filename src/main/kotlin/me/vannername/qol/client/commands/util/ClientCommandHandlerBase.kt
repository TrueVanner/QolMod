...
import me.vannername.qol.client.utils.ClientUtils.sendMessage
import me.vannername.qol.client.utils.ClientUtils.sendSimpleMessage
...
abstract class ClientCommandHandlerBase(val commandName: String) {

    // stores the suggestion providers used for this command.
    private var suggestionProviders: Map<SuggestionProviderKey,
            SuggestionProvider<FabricClientCommandSource>> = mutableMapOf()

    // stores the command nodes (before building) that make up the command.
    private var commandNodes: Map<CommandNodeKey,
            CommandNode<FabricClientCommandSource>> = mutableMapOf()

    // The root node. Must be defined during compile time.
    private lateinit var rootNode: CommandNode<FabricClientCommandSource>
    private var defaultAction: Command<FabricClientCommandSource>? = null
    ...
    protected fun setDefaultAction(action: Command<FabricClientCommandSource>) {
    ...
    protected fun SuggestionProviderKey.getProvider(): SuggestionProvider<FabricClientCommandSource> {
     ...
     */
    protected fun CommandNodeKey.getNode(): CommandNode<FabricClientCommandSource> {
    ...
    protected fun registerSuggestionProvider(
        ...
        provider: SuggestionProvider<FabricClientCommandSource>
    ) {
...
//        return key
     ...
     */
    protected fun ArgumentBuilder<FabricClientCommandSource, *>.register(key: CommandNodeKey, required: Boolean = true) {
        val node = this.build()
    ...
    private fun pathToString(path: List<CommandNodeKey>): String {
        ...
        for (node in path) {
            toReturn += when {
                node.getNode() is LiteralCommandNode<*> -> node.getNode().name
                else -> {
                    when {
    ...
    protected open fun help(ctx: CommandContext<FabricClientCommandSource>): Int {
...
//            ctx.simpleMessage("${} - ${descr.value}", Formatting.YELLOW)
     ...
     */
    @Throws(CommandSyntaxException::class)
    protected open fun run(ctx: CommandContext<*>): Int {
